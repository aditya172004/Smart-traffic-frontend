  ...
  const directions = [
    {
      ...
      label: "North",
      icon: "↑",
      gradient: "bg-gradient-north",
      color: "border-direction-north/30"
    },
    {
      key: "south",
      label: "South",
      icon: "↓",
      gradient: "bg-gradient-south",
      color: "border-direction-south/30"
    },
    {
      key: "east",
      label: "East",
      icon: "→",
      gradient: "bg-gradient-east",
      color: "border-direction-east/30"
    },
    {
      key: "west",
      label: "West",
      icon: "←",
      gradient: "bg-gradient-west",
      color: "border-direction-west/30"
    }
  ];
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      {directions.map((direction, index) => (
        <Card
          key={direction.key}
          className={`${direction.color} border-2 border-dashed bg-card backdrop-blur-sm transition-all duration-300 hover:shadow-glow hover:scale-105 animate-fade-in`}
          style={{ animationDelay: `${index * 0.1}s` }}
        >
          <CardContent className="p-6">
            <div className="text-center">
              <div
                className={`inline-flex items-center justify-center w-16 h-16 rounded-full ${direction.gradient} mb-4 shadow-lg animate-float`}
              >
                <span className="text-2xl font-bold text-white drop-shadow-md">
                  {direction.icon}
                </span>
              </div>
              <h3 className="font-semibold text-lg mb-4 text-card-foreground bg-clip-text text-transparent">
                {direction.label}
              </h3>
