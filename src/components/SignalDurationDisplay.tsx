...
const SignalDurationDisplay = ({ durations }: { durations: Record<string, number> }) => {
  const directions = [
    { label: "north", key: "green_time_N", icon: "↑", gradient: "bg-gradient-north", textColor: "text-direction-north" },
    { label: "south", key: "green_time_S", icon: "↓", gradient: "bg-gradient-south", textColor: "text-direction-south" },
    { label: "east", key: "green_time_E", icon: "→", gradient: "bg-gradient-east", textColor: "text-direction-east" },
    { label: "west", key: "green_time_W", icon: "←", gradient: "bg-gradient-west", textColor: "text-direction-west" }
  ];

  console.log(durations.prediction);
  const totalCycle = Math.round(Object.values(durations.prediction).reduce((sum: number, duration: number) => sum + duration, 0));
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                <div className={`text-3xl font-bold ${direction.textColor} drop-shadow-sm`}>
                  {Math.round(durations.prediction[direction.key] || 0)}s
                </div>
